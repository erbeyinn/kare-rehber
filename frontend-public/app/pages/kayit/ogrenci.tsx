...
const schema = z.object({
  ...
  parent_first_name: z.string().min(1, "Veli adı gerekli"),
  parent_last_name: z.string().min(1, "Veli soyadı gerekli"),
  parent_phone: z.string().min(7, "Veli telefonu gerekli"),
  parent_birthdate: z.string().regex(dateRx, "Veli doğum tarihi YYYY-AA-GG"),
});
...
export default function OgrenciKayit() {
  ...
  if (done) {
    return (
      <Shell
        eyebrow="Kayıt Alındı"
        title="Teşekkürler — kaydınız bize ulaştı."
        lede="Telefon görüşmesi sonrasında kesin kaydınız onaylanır. Giriş bilgileriniz SMS ile iletilecektir."
      >
  ...
  return (
    <Shell
      ...
      lede="Bilgilerinizi eksiksiz doldurun. Kayıt onayı sonrası size ve velinize SMS ile giriş bilgileri iletilir."
    >
      <form onSubmit={onSubmit} className="space-y-8">
        <Section title="Öğrenci" index={1}>
          <Grid>
        ...
        <Section title="Veli" index={2}>
          <Grid>
            <Field label="Ad" error={errors.parent_first_name?.message}>
              <Input {...register("parent_first_name")} />
            </Field>
            <Field label="Soyad" error={errors.parent_last_name?.message}>
              <Input {...register("parent_last_name")} />
            </Field>
            <Field label="Telefon" error={errors.parent_phone?.message}>
              <Input placeholder="+90555..." {...register("parent_phone")} />
            </Field>
            <Field label="Doğum Tarihi" error={errors.parent_birthdate?.message}>
              <Input type="date" {...register("parent_birthdate")} />
            </Field>
          </Grid>
        </Section>
...
function Shell({ eyebrow, title, lede, children }: ShellProps) {
  return (
    <main className="relative min-h-screen overflow-hidden bg-(--color-surface-sunken)">
      <AnimatedMesh tone="warm" className="opacity-50" />
