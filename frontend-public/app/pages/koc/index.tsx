...
import { coachStudents } from "~/api/meetings";
import {
  PageHero,
  SectionHeader,
  EmptyState,
...
export default function KocIndex() {
  const { user } = useAuth();
  ...
  return (
    <div className="max-w-5xl">
      <PageHero
        eyebrow="Hoş geldin"
        title={`${user?.first_name ?? ""} ${user?.last_name ?? ""}`.trim() || "Koç"}
        description="Aşağıda atandığın öğrenciler listeleniyor. Bir öğrenciye tıklayarak görüşme geçmişini görüntüleyebilir veya yeni görüşme ekleyebilirsin."
      />
              ...
              <StaggerItem key={s.id}>
                ...
                >
                  ...
                  >
                    ...
                    <motion.div
                      className="relative text-xs font-medium text-(--color-text-faint) transition-colors group-hover:text-accent"
                      whileHover={{ x: 4 }}
                    >
                      Aç →
                    </motion.div>
